...
3. **Wait for Pass 1 and determine CLAUDE.md targets:**
   ...
   - Use **AskUserQuestion** to confirm targets:
     ```
     questions:
       - question: "[N] CLAUDE.md targets across [M] layers. Proceed with analysis?"
         header: "Targets"
         multiSelect: false
         options:
           - label: "Proceed (Recommended)"
             description: "Analyze all proposed folders and write CLAUDE.md files"
           - label: "Add folders"
...
Read the full template at `${CLAUDE_SKILL_DIR}/templates/root-claude-md.md`.
...
See `${CLAUDE_SKILL_DIR}/examples/root-nodejs-monorepo.md` (Node.js monorepo) and `${CLAUDE_SKILL_DIR}/examples/root-dotnet-clean-arch.md` (.NET Clean Architecture) for well-formed root CLAUDE.md examples.
...
Read the full template at `${CLAUDE_SKILL_DIR}/templates/subfolder-claude-md.md`.
